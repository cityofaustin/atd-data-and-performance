...
const CheckBoxFilters = ({ filters, setFilters }) => {
  const onChange = (filter) => {
    ...
    currentFilters.some((f) => {
      // we use .some() here because the loop break after the first matching filter is found
      if (f.key == filter.key) {
        f.checked = !f.checked;
        return true;
      }
...
export default function Viewer() {
  ...
  React.useEffect(() => {
    ...
    if (currentFilters && currentGeojson && currentGeojson.features) {
    ...
    <>
      ...
      <div className="bg-secondary px-5">
        <div className="bg-primary">
          <Row className="bg-warning mx-5">
            <Col
              className={`${styles["side-pane"]} m-3 h-100 bg-danger`}
              xs={12}
              md={6}
              lg={4}
            >
