...
var requests_url =
  "https://data.austintexas.gov/resource/f6qu-b7zb.json?$limit=10000";
          ...
          render: function (data, type, full, meta) {
            if (full.eval_type == "TRAFFIC") {
              return (
                ...
                default_style[full.eval_type]["fillColor"] +
                ";' ><i class='fa fa-car' ></i> " +
                full.eval_type +
                "</span>"
              );
...
function transitionInfoStat(selection, options) {
  var t = d3.transition().ease(options.ease).duration(options.duration);
