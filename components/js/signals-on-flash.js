...
var t1 = d3.transition().ease(d3.easeQuad).duration(500);

var t2 = d3.transition().ease(d3.easeQuad).duration(500);
  ...
  if (dataset.length > 0) {
    ...
    for (var i = 0; i < dataset.length; i++) {
      if (dataset[i].location) {
        ...
        var status_time = formatDateTime(
          new Date(dataset[i].operation_state_datetime)
        );
...
function adjustMapHeight() {
  ...
  if (signals_on_flash_layer) {
    map.fitBounds(signals_on_flash_layer.getBounds(), {
...
function formatDuration(datetime) {
  ...
  var minutes = parseInt(
    Math.floor((delta_seconds - 86400 * days - 3600 * hours) / 60)
  );
